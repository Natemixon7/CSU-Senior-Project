...
def getProductInfo(soup, arrParts):
    info = soup.find_all('div', {'class': 'item-cell'})
    for item in info:
        price = item.find('span', {'class': 'price-current-label'})
        if price == None:
            continue
        else:
            arrParts.append(item.find('a', {'class': 'item-title'}).get_text())
            arrParts.append(float(item.find('li', {'class': 'price-current'}).strong.get_text().replace(',', '', 1) + item.find('li', {'class': 'price-current'}).sup.get_text()))  # may need to add str
...
def runPages(url, arrParts):
    ...
    if not soup.find('div', {'class': 'btn-group'}):
        url = str(soup.find('div', {'class': 'list-tools-bar'}).contents[-1]['href'])
